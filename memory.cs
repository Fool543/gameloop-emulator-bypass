...
namespace Memory
{
    ...
    public partial class Mem
    {
        ...
        public bool ModifyProtection(string identifier, MemoryProtection newMemProtection, out MemoryProtection previousMemProtection, string filePath = "")
        {
            UIntPtr codeAddress = GetCode(identifier, filePath);
            if (codeAddress == UIntPtr.Zero
                || mProc.Handle == IntPtr.Zero)
            {
                previousMemProtection = default;
                return false;
            }

            return VirtualProtectEx(mProc.Handle, codeAddress, (IntPtr)(mProc.Is64Bit ? 8 : 4), newMemProtection, out previousMemProtection);
        }
